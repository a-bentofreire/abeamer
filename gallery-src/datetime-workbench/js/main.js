...
$(window).on("load", () => {
  ...
  $("#story").html(dates.map((_label, sceneIndex) => `<div class="abeamer-scene" id=scene${sceneIndex}></div>`).join("\n"));
  const animations = [];
  const vars = {};
  ...
  dates.forEach((_date, sceneIndex) => {
    ...
    scene.addAnimations(animations[sceneIndex]).addStills(`${wait}s`);
