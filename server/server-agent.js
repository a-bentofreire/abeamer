...
var server_agent_exports = {};
__export(server_agent_exports, {
  ServerAgent: () => ServerAgent
});
module.exports = __toCommonJS(server_agent_exports);
...
var import_server_consts = require("../shared/lib/server-consts.js");
var import_sprintf = require("../shared/lib/sprintf.js");
var import_plugin_injector = require("./plugin-injector.js");
var ServerAgent;
((ServerAgent2) => {
  const sc = import_server_consts.ServerConsts._SRV_CNT;
  const sprintf = import_sprintf.Sprintf.sprintf;
  ...
  ServerAgent2.toPosixSlash = toPosixSlash;
  ...
  class BaseServer {
    constructor(serverName, args, platform, cwd, existsSync, isDirectory, unlinkSync, mkdirpSync, readUtf8Sync, writeFileSync, posixPathJoin) {
      ...
      this.reportFileName = import_opts_parser.OptsParser.DEFAULT_OUT_REPORT;
      this.width = import_rel_consts.RelConsts.DEFAULT_WIDTH;
      this.hasArgWidth = false;
      this.height = import_rel_consts.RelConsts.DEFAULT_HEIGHT;
      this.hasArgHeight = false;
      this.timeout = 0;
      this.maxWidth = import_opts_parser.OptsParser.DEFAULT_MAX_WIDTH;
      this.maxHeight = import_opts_parser.OptsParser.DEFAULT_MAX_HEIGHT;
      this.maxFps = import_opts_parser.OptsParser.DEFAULT_MAX_FPS;
      this.maxFrameCount = import_opts_parser.OptsParser.DEFAULT_MAX_FRAME_COUNT;
      this.progress = import_opts_parser.OptsParser.DEFAULT_PROGRESS;
      this.url = "";
      this.framesPattern = "";
      this.projDir = "";
      this.fps = 1;
      this.logLevel = import_consts.Consts.LL_WARN;
    ...
    getSetupVars() {
      return this.argsVars ? Object.keys(this.argsVars).map(
        (key) => sc.RENDER_VAR_SUFFIX + encodeURIComponent(`${key}=${this.argsVars[key]}`)
      ).join("&") + "&" : "";
    }
    getPageUrl() {
      let pageUrl = this.url + (this.url.indexOf("?") === -1 ? "?" : "&") + sc.LOG_LEVEL_SUFFIX + this.logLevel + "&" + (this.hasArgWidth ? `${sc.WIDTH_SUFFIX}${this.width}&` : "") + (this.hasArgHeight ? `${sc.HEIGHT_SUFFIX}${this.height}&` : "") + this.getSetupVars() + sc.TELEPORT_SUFFIX + this.toTeleport.toString();
      if (this.logLevel !== 0) {
      ...
      if (cfgConfigData) {
        ...
        if (pCfg) {
          ...
          Object.keys(pCfg).forEach((key) => {
            switch (key) {
              case "plugins":
                import_plugin_injector.PluginInjector.plugins = pCfg[key] || [];
                break;
              case "file":
              case "report":
              case "fps":
              ...
              case "height":
              case "width":
                args.push("--" + key, pCfg[key].toString().replace("__PROJDIR__", this.projDir));
                break;
            }
          });
        }
    ...
    parseConfig(args, cfgText, configFileName, argI) {
      return this.parseJsonConfig(
        args,
        configFileName.match(/\.json$/) ? JSON.parse(cfgText) : { config: { abeamer: this.parseIniCfgContent(cfgText) } },
        configFileName,
        argI
      );
    }
    ...
    parseArguments(args) {
      ...
      function setFramesPattern(framesPattern) {
        self.framesPattern = toPosixSlash(framesPattern || import_opts_parser.OptsParser.DEFAULT_OUT_PATTERN).replace("__PROJDIR__", self.projDir);
      ...
      function parseOption(option, value, multipleValue) {
        switch (option) {
          ...
          case "ll":
            self.logLevel = value;
            self.isVerbose = self.logLevel >= import_consts.Consts.LL_VERBOSE;
            if (self.isVerbose) {
          ...
          case "file":
            ...
            if (!self.url) {
              self.url = "file://" + (isWin ? "/" : "") + encodeURI(self.rootPath + filename);
              if (self.isVerbose) {
          ...
          case "progress":
            ...
            break;
          case "allowedPlugins":
            import_plugin_injector.PluginInjector.loadAllowedPlugins(
              value,
              self.existsSync,
              self.readUtf8Sync
            );
            break;
          case "injectPage":
            import_plugin_injector.PluginInjector.injectPage = value;
            if (self.isVerbose) {
          ...
          case "config":
            ...
            try {
              import_opts_parser.OptsParser.iterateArgOpts(
                true,
                () => injectArgs[injectArgI++],
                parseOption
              );
            } finally {
      ...
      const res = import_opts_parser.OptsParser.iterateArgOpts(true, () => args[argI++], parseOption);
      if (res) {
        return res;
      }
      if (this.url) {
        ...
        try {
          ...
          switch (cmd) {
              ...
              if (this.toGenFrames) {
                if (isFirstFrame) {
                  ...
                  if (matches) {
                    ...
                    if (!this.existsSync(framesFolder)) {
                      ...
                      } else if (this.logLevel > import_consts.Consts.LL_SILENT) {
                ...
                takeScreenshot(this.rootPath + frameFileName, () => {
                  ...
                  if (this.progress > 0 && this.frameNr % this.progress === 0) {
                    console.log(new Array(2 + this.frameNr / this.progress).join("\u2592") + ` ${this.frameNr} frames`);
                  }
                  renderDone();
                });
              } else {
            ...
            case sc.MSG_TELEPORT:
              const storyFileName = (this.configPath ? this.configPath : this.rootPath) + "story.json";
    ...
    handleError(msg) {
      console.error(msg);
      if (toExitOnError) {
        this.exitServer(import_opts_parser.OptsParser.ON_ERROR_EXIT_VALUE);
      }
    }
    // ------------------------------------------------------------------------
    //                               Prepare Server
    ...
    prepareServer() {
      import_plugin_injector.PluginInjector.inject(this.existsSync, this.readUtf8Sync, this.writeFileSync);
    ...
    generateReport() {
      if (this.reportFileName && this.toGenFrames) {
        ...
        this.reportFileName = this.reportFileName.replace("__FRAMES_DIR__", framesDir);
  ...
  ServerAgent2.BaseServer = BaseServer;
})(ServerAgent || (ServerAgent = {}));
